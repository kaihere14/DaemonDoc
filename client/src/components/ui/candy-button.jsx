...
const candyButtonClasses = cn(
  ...
  "px-9 py-3 rounded-xl cursor-pointer transition duration-150 ease-out",
  ...
  // Feedback lands on press, and it is a plain scale — the extra rotation read
  // as a gimmick rather than a physical push.
  "active:scale-[0.97]",
...
/**
 * Anchor-rendered CandyButton — same styling, but a real link so it stays
 * crawlable and middle-clickable. Use this instead of nesting the button in
 * an <a>, which is invalid HTML.
 */
export function CandyLink({ className, children, ...props }) {
  return (
    <a
      className={cn(
        candyButtonClasses,
