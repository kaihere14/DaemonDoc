...
| Live log detail | Convex React client          | Read-only reactive subscriptions for per-log message trails |
| Server          | Node.js + Express.js 5       | REST API server                                             |
| Database        | MongoDB + Mongoose           | All persistent data                                         |
| Realtime store  | Convex                       | Transient live log metadata and message details             |
...
├── convex-server/   # Convex project for live log read models
│   └── convex/
│       ├── logs.ts         # Live log mutations and message query
│       ├── http.ts         # Express-facing Convex HTTP actions
...
- `convex-server/convex/logs.ts` — Convex live log functions; `getLogMessages` streams per-log detail messages by `logId`.
- `convex-server/convex/http.ts` — HTTP actions used by Express to mirror log events into Convex.
...
```
logId (UUID), userId, repoName, repoOwner
...
```
logId, userId, repoName, action, status, updatedAt
```
...
- MongoDB `UserLog` remains the authoritative source for the Logs page list and summary metadata.
- Each `UserLog.logId` is mirrored into Convex so the client can subscribe to detail messages by shared UUID.
